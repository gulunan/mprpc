...
// 这里是框架提供给外部使用的，可以发布rpc方法的函数接口，基类的指针可以接收派生类对象
...
void MprpcProvider::Run() {
    ...
    // 把当前rpc节点上要发布的服务全部注册到zk上，让rpcclient可以从zk上发现服务
    // session timeout    30s默认  zkclient API 会建立一个网络I/O线程，以 1/3*timeout 时间发送ping消息，发送到zkserver会重置sessiontimeout
    ZkClient zkCli;
    zkCli.Start();
...
/*
    在框架内部，RpcProvider(Server)和RpcConsumer(Client)协商好之间通信用的protobuf数据类型
    service_name method_name args  定义proto的message类型，进行数据头的序列化和反序列化
                                   service_name method_name args_size(防止粘包)
    16UserServiceLoginzhang san123456

    header_size(4个字节) + header_str + args_str
    10  "10"
    10000  "10000"  按字符串存储长度不固定，按二进制存储
    std::string  insert和copy方法
*/
// 已建立用户的读写事件回调，如果远程有一个rpc服务的调用请求，那么OnMessage方法就会响应
void MprpcProvider::OnMessage(const muduo::net::TcpConnectionPtr& conn, muduo::net::Buffer* buffer, muduo::Timestamp time) {
    ...
    google::protobuf::Closure* done = google::protobuf::NewCallback<MprpcProvider, const muduo::net::TcpConnectionPtr&, google::protobuf::Message*>(this, &MprpcProvider::SendRpcResponse, conn, response);
...
void MprpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr& conn, google::protobuf::Message* response) {
    ...
    if (response->SerializeToString(&response_str)) {
        // 序列化成功后，通过网络把rpc方法执行的结果发送至调用方
        conn->send(response_str);
    } else {
        std::cout << "serialize response_str error!" << std::endl;
        LOG_ERR("serialize response_str error!");
    }
    // 模拟http的短连接服务，由mprpcprovider主动断开连接
    conn->shutdown();
}